...
std::tuple<torch::Tensor, std::vector<torch::Tensor>> wtconv_forward(
    ...
    int stride, int pad, int groups, float dwt_scale, float idwt_scale, bool training = false) 
{
    int levels = weights.size();
    std::vector<torch::Tensor> saved_inputs; 
    if (training) saved_inputs.reserve(levels);
    std::vector<torch::Tensor> processed_highs; processed_highs.reserve(levels);
...
std::tuple<torch::Tensor, std::vector<torch::Tensor>> wtconv_backward(
    ...
    std::vector<torch::Tensor> weights, int groups, float dwt_scale, float idwt_scale) 
{
    int levels = weights.size();
    ...
    for(int i=0; i<levels; ++i) {
        int N = g.size(0); int C = g.size(1); int H = g.size(2)/2; int W = g.size(3)/2;
        auto gc = torch::empty({N, 4*C, H, W}, g.options());
        auto gr = torch::empty({N, C, H, W}, g.options());
        launch_fused_dwt_split(g.data_ptr<float>(), gc.data_ptr<float>(), gr.data_ptr<float>(), N, C, H, W, idwt_scale);
        grad_conv_outs[i] = gc;
        g = gr; // Propagate gradient down to next level's reconstruction input
    }
    ...
    for(int i=levels-1; i>=0; --i) {
        ...
        int N = input_img.size(0); int C = input_img.size(1); int H = input_img.size(2); int W = input_img.size(3);
        int H_sub = H/2; int W_sub = W/2;
        
        // Rematerialize DWT
        auto dwt_coeffs = torch::empty({N, 4*C, H_sub, W_sub}, input_img.options());
        launch_fused_dwt_split(input_img.data_ptr<float>(), dwt_coeffs.data_ptr<float>(), nullptr, N, C, H_sub, W_sub, dwt_scale);
        
        // Weight Grad
        int K = w.size(2);
        auto grad_w = torch::empty_like(w);
        launch_conv_depthwise_bwd_w(grad_conv_outs[i].data_ptr<float>(), dwt_coeffs.data_ptr<float>(), grad_w.data_ptr<float>(), N, 4*C, H_sub, W_sub, K, 1, K/2, H_sub, W_sub);
        ...
        launch_fused_conv_bwd_idwt(grad_conv_outs[i].data_ptr<float>(), g_deep_ptr, w.data_ptr<float>(), grad_input.data_ptr<float>(), N, C, H_sub, W_sub, K, dwt_scale);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("wtconv_forward", &wtconv_forward, "Unified Forward", py::arg("input"), py::arg("weights"), py::arg("stride"), py::arg("pad"), py::arg("groups"), py::arg("dwt_scale"), py::arg("idwt_scale"), py::arg("training")=false);
    m.def("wtconv_backward", &wtconv_backward, "Fused Backward", py::arg("saved_inputs"), py::arg("grad_out"), py::arg("weights"), py::arg("groups"), py::arg("dwt_scale"), py::arg("idwt_scale"));
}